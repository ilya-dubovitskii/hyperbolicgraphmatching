...
from layers.mobius_linear import MobiusLinear
from layers.mobius_layers import MobiusGC
...
class MobiusGraphMatching(torch.nn.Module):
    ...
    def forward(self, x_s, edge_index_s, edge_attr_s, batch_s, x_t,
        ...
        if self.k < 1:
            ...
            S_hat = (tmp_s * tmp_t.view(B, N_s, k, C_out)).sum(dim=-1) - 2 * (
                        tmp_s[..., 0] * tmp_t.view(B, N_s, k, C_out)[..., 0])
...
class MobiusGCN(torch.nn.Module):
    def __init__(self, manifold, in_channels, out_channels, num_layers,
        ...
        for _ in range(self.num_layers):
            self.convs.append(MobiusGC(self.manifold, in_channels, out_channels, dropout=dropout, use_att=use_att,
                                           use_bias=use_bias))
