...
class Config:
    def __init__(self, space, c, out_channels,
                num_layers, cat, lin, dropout, sim, k, batch_size, lr, gamma, max_epochs):
        self.space = space
        self.c = c
    ...
    def save(self, fname):
        ...
        'lin': self.lin, 'dropout': self.dropout, 'k': self.k, 'batch_size': self.batch_size, 'lr': self.lr, 'gamma': self.gamma, 'max_epochs': self.max_epochs}
    ...
    def __repr__(self):
        ...
        lin={self.lin}; k={self.k}, batch_size={self.batch_size}, lr={self.lr}, gamma={self.gamma}, max_epochs={self.max_epochs}'
