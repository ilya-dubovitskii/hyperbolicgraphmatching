...
ass = KFoldAssessment(5, 'ja_en_hyperbolic', parameter_ranges, num_configs=100)
path = os.path.join('..', '..', 'data', 'DBP15K')
data = DBP15K(path, 'ja_en', transform=SumEmbedding())[0]


gt = torch.cat([data.train_y, data.test_y], dim=-1)


data = GraphMatchingDataset('ja_en', data.x1/2, data.edge_index1, None, data.x2/2, data.edge_index2, None, gt)
